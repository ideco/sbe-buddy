...
It reads the header alone and never throws: the schema id must be the schema's, the template id one of the codec's messages, and the version at or above the schema's `baselineVersion`. `decode` checks the same and throws `IllegalArgumentException` on a message that is not its own, naming the union's templates: `not a OrderCommand: schemaId 6, templateId 3; its templates are 1, 2`.
