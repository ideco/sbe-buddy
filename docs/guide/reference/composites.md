...
* An inline enum, set or composite, a component whose type is an `@SbeEnum`, `@SbeSet` or `@SbeComposite` nested in the record. The declaration is written inside the composite in the schema, and the component is the nested type.
